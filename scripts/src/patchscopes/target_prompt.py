...
def create_few_shot_prompt(num_of_tokens, examples, separator='|'): # Create jailbrak prompt
    ...
    separator = " " + separator + " "
    prompt = separator.join(selected_examples) + separator[:-1]  + " x" * num_of_tokens #Just x or description

    # inp = tokenizer(jailbreak_prompt, return_tensors="pt") #jailbreak prompt instead of dummy inp
    # pos = inp['input_ids'].shape[1]

    return prompt

def create_jailbreak_prompts(jailbreak_prompt, tokenizer):
    inp = tokenizer(jailbreak_prompt, return_tensors="pt") #jailbreak prompt instead of dummy inp
    prompt = inp['input_ids'].shape[1] * " x"
